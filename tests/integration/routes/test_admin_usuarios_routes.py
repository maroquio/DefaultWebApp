...
class TestCadastrarUsuario:
    ...
    def test_cadastrar_usuario_com_perfil_autor(self, admin_autenticado):
        """Admin deve poder cadastrar autor"""
        response = admin_autenticado.post("/admin/usuarios/cadastrar", data={
            "nome": "Novo Autor",
            "email": "novoautor@example.com",
            "senha": "SenhaAutor@123",
        ...
        # Verificar perfil
        from repo import usuario_repo
        usuario = usuario_repo.obter_por_email("novoautor@example.com")
        assert usuario is not None
...
class TestEditarUsuario:
    ...
    def test_editar_usuario_com_dados_validos(self, admin_autenticado, criar_usuario):
        ...
        assert usuario_editado.perfil == Perfil.AUTOR.value
