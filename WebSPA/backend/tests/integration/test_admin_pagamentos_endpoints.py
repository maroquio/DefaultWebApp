...
class TestListar:
    ...
    def test_item_shape(self, admin_autenticado):
        _inserir_pagamento(
    ...
    def test_total_e_total_paginas(self, admin_autenticado):
        # 25 pagamentos, 10 por página -> 3 páginas
        for i in range(25):
            _inserir_pagamento(descricao=f"Pag {i}")
        resp = admin_autenticado.get("/api/admin/pagamentos")
        assert resp.status_code == status.HTTP_200_OK
        corpo = resp.json()
    ...
    def test_segunda_pagina(self, admin_autenticado):
        for i in range(25):
            _inserir_pagamento(descricao=f"Pag {i}")
    ...
    def test_ultima_pagina_parcial(self, admin_autenticado):
        for i in range(25):
            _inserir_pagamento(descricao=f"Pag {i}")
    ...
    def test_por_pagina_customizado(self, admin_autenticado):
        for i in range(12):
            _inserir_pagamento(descricao=f"Pag {i}")
...
class TestListarFiltroStatus:
    def test_filtro_status_valido(self, admin_autenticado):
        _inserir_pagamento(descricao="A", status_pag=StatusPagamento.APROVADO)
        _inserir_pagamento(descricao="B", status_pag=StatusPagamento.APROVADO)
        _inserir_pagamento(descricao="C", status_pag=StatusPagamento.PENDENTE)
        _inserir_pagamento(descricao="D", status_pag=StatusPagamento.RECUSADO)
    ...
    def test_filtro_status_sem_correspondencia(self, admin_autenticado):
        _inserir_pagamento(status_pag=StatusPagamento.PENDENTE)
        resp = admin_autenticado.get(
    ...
    def test_filtro_status_inexistente_ignorado(self, admin_autenticado):
        # status_filtro inválido (não existe no enum) é ignorado -> retorna todos
        _inserir_pagamento(status_pag=StatusPagamento.PENDENTE)
        _inserir_pagamento(status_pag=StatusPagamento.APROVADO)
        resp = admin_autenticado.get(
...
class TestDetalhes:
    ...
    def test_detalhes_sem_payment_id_nao_consulta_provider(self, admin_autenticado):
    ...
    def test_detalhes_com_payment_id_consulta_provider(self, admin_autenticado):
    ...
    def test_detalhes_provider_falha_degrada_para_none(self, admin_autenticado):
