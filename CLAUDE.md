...
**User Profiles (Roles):**
- **NEVER use string literals** for profiles (e.g., "admin", "autor")
- **ALWAYS use** `Perfil` enum from `util/perfis.py`: `Perfil.ADMIN.value`, `Perfil.AUTOR.value`, `Perfil.LEITOR.value`
...
- Base class for all domain enums (`Perfil`, `StatusArtigo`, `StatusChamado`, `PrioridadeChamado`, etc.)
...
- Helper methods:
  ...
  - `is_autor()`: Check if user is autor
  - `is_leitor()`: Check if user is leitor
...
- Default users created from `data/usuarios_seed.json`:
  - padrao@administrador.com / 1234aA@# (Administrador)
  - padrao@autor.com / 1234aA@# (Autor)
  - padrao@leitor.com / 1234aA@# (Leitor)
...
- `dtos/categoria_dto.py`: Blog category validation
- `dtos/artigo_dto.py`: Blog article validation
...
- `model/categoria_model.py`: Blog category entity
- `model/artigo_model.py`: Blog article entity with `StatusArtigo` enum (`EnumEntidade`)
...
- `sql/categoria_sql.py`, `sql/artigo_sql.py`
...
- `repo/categoria_repo.py`, `repo/artigo_repo.py`: Blog CRUD repositories
...
- `routes/admin_categorias_routes.py`: Blog categories CRUD (prefix: `/admin/categorias`, admin-only)
- `routes/artigos_routes.py`: Blog articles CRUD (prefix: `/artigos`, author)
